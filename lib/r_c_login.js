...
return res.status(400).send({error:1,param:'idUsuario'});cnf.ejecutarResSQL(SQL_BUSCAR_USUARIOS,[celular,idUsuario],function(usuarios){if(usuarios.length<=0)
return res.status(200).send({en:-1,m:'No existen miembres registrados '});return res.status(200).send({en:1,usuarios:usuarios[0]});},res);}
const SQL_BUSCAR_USUARIOS="SELECT p.id_persona,p.nombres,p.apellidos, p.telefono,pr.usuario,pr.idUsuario FROM bancodt.persona p inner join usuario pr on pr.id_persona=p.id_persona where pr.id_rol=2 and p.telefono=? and pr.idUsuario <> ?"
