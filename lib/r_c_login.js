...
const SQL_AUTENTICAR="SELECT p.id_persona,p.nombres,p.apellidos, p.telefono,pr.usuario FROM bancodt.persona p inner join usuario pr on pr.id_persona=p.id_persona where pr.id_rol=2 and usuario=? and password = ?"
