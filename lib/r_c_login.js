...
return res.status(400).send({error:1,param:'valoracion'});cnf.ejecutarResSQL(SQL_BUSCAR_USUAURIO,[monto,idUsuarioPaga],function(usuarios){if(usuarios.length<=0)
...
const SQL_BUSCAR_USUAURIO="SELECT idUsuario FROM bancodt.usuario where tiempo >=? and idUsuario =?;"
const SQL_insertar_transaccion="INSERT INTO bancodt.transacciones_tiempo (numero_horas, descripcion_actividad, id_ofertante, id_demandante, valoracion) VALUES (?, ?, ?, ?, ?);";module.exports=router;