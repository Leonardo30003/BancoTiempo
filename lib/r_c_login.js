...
const SQL_AUTENTICAR="SELECT p.id_persona,p.nombres,p.apellidos, p.telefono,pr.usuario,pr.idUsuario,pr.tiempo,p.direccion,p.email,p.imagen,pr.calificacion FROM bancodt.persona p inner join usuario pr on pr.id_persona=p.id_persona where pr.id_rol=2 and usuario=? and password = ?"
...
const SQL_BUSCAR_USUARIOS="SELECT u.idUsuario,p.nombres,p.apellidos,p.email,p.imagen,p.telefono,u.usuario,u.calificacion,u.tiempo FROM bancodt.persona p INNER JOIN bancodt.usuario u ON u.id_persona = p.id_persona WHERE u.id_rol = 2 AND u.idUsuario <> ? AND idUsuario <> 1"
